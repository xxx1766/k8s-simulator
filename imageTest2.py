...
# PAYLOADJSON = "/root/k8s-simulator/9-jobs-info/payload.json"
...
PAYLOADJSON = f"/root/k8s-simulator/{JOBNUM}-jobs-info/payload.json"
...
def createPodAndAutoDelete(
    ...
    nodeID = waitForPodScheduled(podStart, podName, image, timeoutSeconds=30)
    if nodeID is None:
...
if __name__ == "__main__":
    ...
    f = open(f"steady-image-bw{args.bw}-node{args.node}-test{args.test}.log",'w')
    # testfile = f"/root/k8s-simulator/trace/2017-9-25-Simulation-{args.test}jobs.json"
    testfile = f"/root/k8s-simulator/trace/4000/steady_trace_{args.test}.json"
    ...
    catalog = buildLayerCatalog(loadJSON(PAYLOADJSON))
    ...
    print(f"image-bw{args.bw}-node{args.node}-test{args.test}-Completed!")
    time.sleep(2)
    f.flush()
